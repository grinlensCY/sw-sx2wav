...
try:
    import file_driver_sxr2sx as FD
except:
    from . import file_driver_sxr2sx as FD
...
class Protocol:
    ...
    def __init__(self,drv,dec_file=None,key='',iv=''):
        self.driver=drv
        ...
        if(type(drv) is FD.Driver and drv.is_encrypted()==False):
        ...
        static uint8_t protocol_iv_key[16] = {'S', 'i', 'r', 'i', 'u', 'X', 'e', 'n', 
                                            's', 'e', '2', '1', '0', '0', 'F', 'w'};
        '''
        self.key='SiriuXense2100Fw'.encode('ASCII') if not key else key.encode('ASCII')
        self.iv= 'akWLytV$N-_X:2zK'.encode('ASCII') if not iv else iv.encode('ASCII')
        print(f"key={key}={self.key.decode()}   iv={iv}={self.iv.decode()}")
    ...
    def __prase_dual_mic_pkg(self,pkg):
        ...
        for _ in range(batch_cnt):
            ...
            #20220110: dual的輸出是env/main排列，先前52832版本也是這樣
            main.append(val[1])
            env.append(val[0])

        ts=pkg[1]

        if(self.pre_mic_ts!=0):
            diff=ts-self.pre_mic_ts
            #print(diff)#about 512
            if(diff>700):
                print('missing mic')
    ...
    def start(self):
        ...
        self.thd=threading.Thread(target = self.__decode_thd_fun, args =(self.thd_run_flag,self.driver,self.tx_queue,self.rx_queue,))
        self.thd.start()

        self.auto_prase_thd=threading.Thread(target = self.__auto_prase_thd_fun, args =(self.thd_run_flag,self.rx_queue,self.cmd_resp_queue,))
...
def findFileset(config, kw='audio-main',srcdir=''):
    ...
    tfn = filedialog.askopenfilename(initialdir=sdir,filetypes=[("SXR File",(f"*.sxr"))])
    if not tfn:
        return []
    srcdir = os.path.dirname(tfn)

    fns = [f"{srcdir}/{fn}" for fn in os.listdir(srcdir) if fn.endswith('.sxr')]
...
if __name__ == "__main__":
    ...
    fns = findFileset(config,srcdir=sdir)

    for src in fns:
        keyfn = src.replace(".sxr","_keyiv.txt")
        if os.path.exists(keyfn):
            ...
            key = iv = ''
        # src='./sample/1655992722004.sxr'
        dst=src.replace('.sxr','sx')

        drv = FD.Driver(src)

        protocal=Protocol(drv,dst,key=key,iv=iv)
        protocal.start()
        ...
        while(drv.is_finished()==False):
            time.sleep(0.1)
        protocal.wait_proc()

        protocal.stop()
        print(f'{src} is converted to sx')

'''
if __name__ == "__main__":
    src='./sample/1655992722004.sx'
    drv = FD.Driver(src)
